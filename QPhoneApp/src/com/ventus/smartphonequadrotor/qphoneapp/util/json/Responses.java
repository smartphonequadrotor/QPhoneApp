...
public class Responses {
	...
	public Responses(TriAxisSensorResponse[] orientation,
			TriAxisSensorResponse[] gyro, TriAxisSensorResponse[] accel,
			TriAxisSensorResponse[] mag, HeightSensorResponse[] height,
			CameraResponse[] camera, BatteryResponse[] battery,
			GpsResponse[] gps, String systemState, String debug) {
	...
	public String toString() {
		return String
				.format("Orientation: %8$s\nGyro: %1$s\nAccel: %2$s\nMagnetometer: %9$s\nHeight: %10$s"
						+ "Camera: %3$s\nBattery: %4$s\nGps: %5$s\nSystemState: %6$s\nDebug: %7$s\n",
						Gyro.toString(), Accel.toString(), Camera.toString(),
						Battery.toString(), GPS.toString(), SystemState, Debug,
						Orientation.toString(), Mag.toString(), Height);
	}
}